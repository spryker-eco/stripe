<?php

/**
 * MIT License
 * For full license information, please view the LICENSE file that was distributed with this source code.
 */

namespace SprykerEco\Yves\Stripe\Plugin\StepEngine;

use SprykerEco\Yves\Kernel\AbstractPlugin;
use SprykerEco\Yves\StepEngine\Dependency\Form\SubFormInterface;
use SprykerEco\Yves\StepEngine\Dependency\Plugin\Form\SubFormPluginInterface;
use SprykerEco\Yves\Stripe\Form\DataProvider\StripeInvoiceDataProvider;

/**
 * @method \SprykerEco\Yves\Stripe\StripeFactory getFactory()
 */
class StripeInvoiceSubFormPlugin extends AbstractPlugin implements SubFormPluginInterface
{
    public function createSubForm(): SubFormInterface
    {
        return $this->getFactory()->createStripeInvoiceSubForm();
    }

    public function createSubFormDataProvider(): StripeInvoiceDataProvider
    {
        return $this->getFactory()->createStripeInvoiceDataProvider();
    }
}
